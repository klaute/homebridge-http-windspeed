  ...
  async heartbeat(beat) {
    ...
    if (beat % heartrate === 1) {
      try {
        ...
        const tmp_json_data = result.body
        this.log("WsAccessory:71: result.body = "+result.body)
        try {
          this.json_data = JSON.parse(tmp_json_data.replace(/ /g, ''))

        } catch (parseErr) {
          this.log('WsAccessory:76: Error processing received information: ' + parseErr.message);
          try {
            this.json_data = JSON.parse(tmp_json_data.replace(/ /g, '')).weather

          } catch (parseErr) {
            this.log('WsAccessory:81: Error processing received information: ' + parseErr.message);
        ...
        for (const id in this.wsServices) {
          if (typeof this.wsServices[id].checkObservation === 'function') {
            ...
            this.log("WsAccessory:90: json_data=" + this.json_data)
            this.wsServices[id].checkObservation(this.json_data)
