  ...
  async heartbeat(beat) {
    ...
    if (beat % heartrate === 1) {
      try {
        ...
        try {
          this.json_data = result.body

        } catch (parseErr) {
          this.log('WsAccessory:72: Error processing received information: ' + parseErr.message);
          try {
            this.json_data = result.body.weather

          } catch (parseErr) {
            this.log('WsAccessory:77: Error processing received information: ' + parseErr.message);
        ...
        for (const id in this.wsServices) {
          if (typeof this.wsServices[id].checkObservation === 'function') {
            ...
            this.log("WsAccessory:86: json_data=" + this.json_data)
