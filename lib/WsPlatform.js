...
class WsPlatform extends homebridgeLib.Platform {
  constructor(log, configJson, homebridge, bridge) {
    ...
    try {
      ...
      this._client
        .on('error', (error) => {
          this.log(
            'WsPlatform:65: error: request %d: %s; %s', error.request.id,
            error.request.method, error.request.resource
          )
          this.warn(
            'WsPlatform:69: request %d: %s', error.request.id, error
          )
        })
        .on('request', (request) => {
          this.debug(
            'WsPlatform:74: request %d: %s; %s', request.id,
            request.method, request.resource
          )
          this.vdebug(
            'WsPlatform:78: request %d: %s; %s', request.id,
            request.method, request.url
          )
        })
        .on('response', (response) => {
          this.vdebug(
            'WsPlatform:84: request %d: response: %j', response.request.id,
            response.body
          )
          this.debug(
            'WsPlatform:88: request %d: %d; %s', response.request.id,
  ...
  async weather(location) {
    ...
    this.log("WsPlatform:158: %s : async weather call", this.config.name)
    const response = await this._client.get(this.config.suffix)
    this.vdebug("%s : async weather response = %s", this.config.name, response)
    this.log("WsPlatform:162: %s : async weather response = %s", this.config.name, response)
    ...
    if (response.body.cod !== 200) {
      ...
      this.log(
        'WsPlatform:170: request %d: %s %s', error.request.id,
        error.request.method, error.request.resource
      )
      this.warn(
        'WsPlatform:174: request %d: %s', error.request.id, error
      )
      throw error
    }
    return response
