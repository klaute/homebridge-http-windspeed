...
class WsPlatform extends homebridgeLib.Platform {
  constructor (log, configJson, homebridge, bridge) {
    ...
    try {
      ...
      this.log("host: %s", this.config.host)
      this.log("path: %s", this.config.path)
      this.log("suffix: %s", this.config.suffix)
      this.log("name: %s", this.config.name)
      this.log("manufacturer: %s", this.config.manufacturer)
      this.log("model: %s", this.config.model)
      this.log("serial: %s", this.config.serial)
      this.log("timeout: %d", this.config.timeout)
      this.log("min windspeed: %d", this.config.min_windspeed)
      this.log("max windspeed: %d", this.config.max_windspeed)
  ...
  async windspeed () {
    this.log("%s : async windspeed start", this.config.name)
    const response = await this._client.get(this.config.suffix)
    this.log("%s : async windspeed finish", this.config.name)
    this.log("%s : async windspeed data = %s", this.config.name, response)
  ...
  async onecall () {
    this.log("%s : onecall start", this.config.name)
    this.config.recv_data = this._client.get(this.config.suffix)
    this.log("%s : onecall finish", this.config.name)
