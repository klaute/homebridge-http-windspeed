...
class WsPlatform extends homebridgeLib.Platform {
  constructor (log, configJson, homebridge, bridge) {
    ...
    try {
      ...
      this._client = new homebridgeLib.HttpClient({
        https: false,
        host: '192.168.0.161:8080', // TODO this.config.host,
        json: true,
        maxSockets: 1,
        path: '/', // TODO this.config.path,
  ...
  async weather (location) {
    ...
    const response = await this._client.get('/hbWebstat') // TODO this.config.suffix
  ...
  async onecall () {
    ...
    this.config.recv_data = this._client.get('/hbWebstat') // TODO this.config.suffix
