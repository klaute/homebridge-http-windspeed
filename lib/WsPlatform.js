...
//
// Removed all except of Windspeed, Temperature, Preassure and Humidity and applied some other config settings
...
class WsPlatform extends homebridgeLib.Platform {
  constructor (log, configJson, homebridge, bridge) {
    ...
    this.config = {
      name: 'Homebridge HTTP Weatherstation',
      timeout: 15,
      min_windspeed: 0,
      max_windspeed: 10000,
    }
    ...
    try {
      ...
      this._client = new homebridgeLib.HttpClient({
        ...
        suffix: '', // will be added in async weatherstation/onecall
  ...
  async weatherstation () {
    this.vdebug("%s : async weatherstation", this.config.name)
    const response = await this._client.get(this.config.suffix)
    this.vdebug("%s : async weatherstation response = %s", this.config.name, response)
