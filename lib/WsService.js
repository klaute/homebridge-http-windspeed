...
class Temperature extends WsService {
  ...
  checkObservation(observation) {
    const o = observation
    this.vdebug('WsService:62: observation: %j', o)
...
class Humidity extends WsService {
  ...
  checkObservation(observation) {
    const o = observation
    this.vdebug('WsService:82: observation: %j', o)
...
class Weather extends WsService {
  ...
  checkObservation(observation) {
    const o = observation
    this.vdebug('WsService:223: observation: %j', o)
    if (o == null) {
      return
    }
    this.log("WsService:227: temperature="+o.temperature)
...
class DailyForecast extends WsService {
  ...
  checkObservation(observation) {
    ...
    this.vdebug('WsService:382: observation: %j', o)
    if (o == null) {
      return
    }
...
class HourlyForecast extends WsService {
  ...
  checkObservation(observation) {
    const o = observation
    this.vdebug('WsService:517: observation: %j', o)
    if (o == null) {
      return
    }
    this.log("WsService:521: temperature="+o.temperature)
    this.values.apparentTemperature = Math.round(o.temperature * 10) / 10
    this.values.clouds = 0
    this.values.condition = 0
    this.values.conditionCategory = 0
